#!/usr/bin/env sh
# ==============================================================================
# install-binary.sh — install a prebuilt l0-cache binary (no Rust toolchain).
#
...
#
# Env overrides: L0_CACHE_BIN_DIR (install dir), L0_CACHE_VERSION (tag, e.g. v0.1.8).
# ==============================================================================
set -eu

REPO="fabriziosalmi/l0-cache"
BIN_DIR="${L0_CACHE_BIN_DIR:-$HOME/.local/bin}"
...
case "$os-$arch" in
  ...
  *)
    ...
    err "Build from source instead:  git clone https://github.com/$REPO && cd l0-cache && ./install.sh"
    exit 1
    ;;
esac

# 2. Resolve the release tag (latest unless pinned via L0_CACHE_VERSION).
if [ -n "${L0_CACHE_VERSION:-}" ]; then
  tag="$L0_CACHE_VERSION"
else
...
printf 'Downloading l0-cache %s (%s)…\n' "$tag" "$target"
...
# 4. Install + create the `t` alias.
tar -xzf "$tmp/$art" -C "$tmp"
mkdir -p "$BIN_DIR"
install -m 0755 "$tmp/l0-cache" "$BIN_DIR/l0-cache"
ln -sf l0-cache "$BIN_DIR/t"

printf 'Installed: %s  →  %s (alias: t)\n' "$("$BIN_DIR/l0-cache" --version)" "$BIN_DIR/l0-cache"
case ":$PATH:" in
  *":$BIN_DIR:"*) ;;
  *) err "Note: $BIN_DIR is not on your PATH — add it to use l0-cache." ;;
esac
