...
`l0-compressor`, so the model never has to prefix anything.
...
| Command | Effect |
|---|---|
| `install` | Write the wrapper to `~/.claude/hooks/l0-compressor-wrapper.sh` and register a `PreToolUse` (matcher `Bash`) hook in `settings.json`. Idempotent; saves a timestamped backup. |
| `enable` / `on` | Create the toggle file `~/.config/l0-compressor/hook.enabled`. Instant. |
| `disable` / `off` | Remove the toggle file. Instant. |
| `status` | Show the installed/registered/enabled state and the `l0-compressor` version. |
...
an updated `tool_input` with `l0-compressor --quiet` prepended to the command. Only the
...
| Already wrapped | commands already starting with `l0-compressor` / `t` |

These are wrapped by `l0-compressor` only when *you* invoke it explicitly; the hook
...
If the toggle file is absent, or `l0-compressor`/`jq` is not on `PATH`, or `jq` fails
...
`l0-compressor` does not store anything to replay — it filters output on the fly. The
...
*without* prefixing `l0-compressor`: if the hook is live, the output comes back
truncated with an `l0-compressor` footer. Then check the savings:

```sh
l0-compressor --stats          # aggregated token savings
...
to prefix noisy read-only commands with `l0-compressor`:
