...
`l0-compressor` propagates the child process exit code as its own. The following
...
When the child process is killed by a signal, `l0-compressor` reports the exit code
...
| 141 | SIGPIPE (13) | `l0-compressor cmd \| head` (pipe closed) |
...
| Code | Meaning |
|---|---|
| 127 | `l0-compressor` itself failed to start the child (command not found, `/bin/sh` missing) |
| 141 | `l0-compressor` detected BrokenPipe on stdout (e.g. `l0-compressor cmd \| head`) |

## Behavior on Signals

The captured child runs in its **own process group**. When `l0-compressor` receives
SIGINT (Ctrl-C) or SIGTERM (`kill`, `timeout`, systemd, `docker stop`):

1. `l0-compressor`'s signal handler **forwards** the signal to the child's process
...
3. `l0-compressor` finishes reading, calls `child.wait()`, and collects the status.
4. `l0-compressor` reports `128 + signal_number` (e.g. 130 for SIGINT, 143 for SIGTERM).

This both lets the child terminate cleanly **and** ensures `l0-compressor` always
completes its own cleanup (metrics logging) before exiting. SIGPIPE is ignored so
that `l0-compressor cmd | head` can log metrics before exiting 141.
