#!/bin/bash
# ==============================================================================
# 'l0-compressor' - CLI Proxy Setup Script
...
print_banner() {
    echo -e "${BOLD}${CYAN}"
    echo "  ┌────────────────────────────────────────────────────────┐"
    echo "  │               l0-compressor  CLI Proxy Setup                │"
...
show_help() {
    print_banner
    echo "Usage:"
    echo "  ./install.sh                  Build and install 'l0-compressor' (interactive)"
    ...
    echo "  ./install.sh --uninstall      Uninstall 'l0-compressor' from the system"
...
uninstall() {
    ...
    if [ -f "$LOCAL_BIN_DIR/$BINARY_NAME" ]; then
        if [ "$DRY_RUN" = true ]; then
            ...
            log_info "[Dry-Run] Would remove local symlinks: $LOCAL_BIN_DIR/l0-comp, $LOCAL_BIN_DIR/t"
        else
            rm "$LOCAL_BIN_DIR/$BINARY_NAME"
            if [ -L "$LOCAL_BIN_DIR/l0-comp" ] || [ -f "$LOCAL_BIN_DIR/l0-comp" ]; then rm "$LOCAL_BIN_DIR/l0-comp"; fi
            if [ -L "$LOCAL_BIN_DIR/t" ] || [ -f "$LOCAL_BIN_DIR/t" ]; then rm "$LOCAL_BIN_DIR/t"; fi
            log_success "Removed local binary and short commands 'l0-comp' and 't'."
        fi
        removed=1
    fi
    ...
    if [ -f "$GLOBAL_BIN_DIR/$BINARY_NAME" ]; then
        if [ "$DRY_RUN" = true ]; then
            ...
            log_info "[Dry-Run] Would remove global symlinks: $GLOBAL_BIN_DIR/l0-comp, $GLOBAL_BIN_DIR/t"
        else
            if [ -w "$GLOBAL_BIN_DIR" ]; then
                rm "$GLOBAL_BIN_DIR/$BINARY_NAME"
                if [ -L "$GLOBAL_BIN_DIR/l0-comp" ] || [ -f "$GLOBAL_BIN_DIR/l0-comp" ]; then rm "$GLOBAL_BIN_DIR/l0-comp"; fi
                if [ -L "$GLOBAL_BIN_DIR/t" ] || [ -f "$GLOBAL_BIN_DIR/t" ]; then rm "$GLOBAL_BIN_DIR/t"; fi
            else
                log_warn "Requesting sudo privileges to delete global files..."
                sudo rm "$GLOBAL_BIN_DIR/$BINARY_NAME"
                if [ -L "$GLOBAL_BIN_DIR/l0-comp" ] || [ -f "$GLOBAL_BIN_DIR/l0-comp" ]; then sudo rm "$GLOBAL_BIN_DIR/l0-comp"; fi
                if [ -L "$GLOBAL_BIN_DIR/t" ] || [ -f "$GLOBAL_BIN_DIR/t" ]; then sudo rm "$GLOBAL_BIN_DIR/t"; fi
            fi
            log_success "Removed global binary and short commands 'l0-comp' and 't'."
        fi
        removed=1
    fi
    ...
    if [ "$DRY_RUN" = true ]; then
        ...
        log_success "Uninstall completed. 'l0-compressor' is no longer present on your system."
    else
        log_warn "No installations of 'l0-compressor' were found in typical directories."
    fi
    exit 0
}
...
perform_install() {
    ...
    if [ "$DRY_RUN" = true ]; then
        log_info "[Dry-Run] Would build 'l0-compressor' in release mode using: cargo build --release"
    else
        mkdir -p target
        echo -e "  ${ICON_GEAR}  Compiling 'l0-compressor'..."
        cargo build --release > "$BUILD_LOG" 2>&1 &
        local cargo_pid=$!
        
        run_with_spinner "$cargo_pid" "Compiling 'l0-compressor' release binary (this may take a moment)..."
    ...
    if [ "$DRY_RUN" = true ]; then
        log_info "[Dry-Run] Would copy 'l0-compressor' to $dest_path"
        log_info "[Dry-Run] Would create short command 'l0-comp' and 't' symlinks in $target_dir"
    else
        log_info "Deploying binary to $dest_path..."
        if [ "$is_global" = true ]; then
            if [ -w "$target_dir" ]; then
                cp "target/release/$BINARY_NAME" "$dest_path"
                ln -sf "$BINARY_NAME" "$target_dir/l0-comp"
                ...
                sudo ln -sf "$BINARY_NAME" "$target_dir/l0-comp"
        ...
        log_success "Deployment completed successfully (including short commands 'l0-comp' and 't')."
    fi
    ...
    if [ "$is_global" = false ]; then
        ...
        if [ "$current_shell" = "fish" ]; then
            ...
            if ! grep -q "$target_dir" "$profile_file"; then
                if [ "$DRY_RUN" = true ]; then
                    ...
                    echo "" >> "$profile_file"
                    echo "# Added by 'l0-compressor' installer" >> "$profile_file"
        ...
        if echo ":$PATH:" | grep -q ":$target_dir:"; then
            ...
            log_warn "$target_dir is not on your \$PATH — 'l0-compressor' may not be found."
...
setup_completions() {
    ...
    "$bin_path" --completions zsh > "$zsh_completion_dir/_l0-compressor"
...
EOF
    ...
    log_success "Zsh completions generated in $zsh_completion_dir/_l0-compressor and _t"

    # Fish
    if command -v fish &>/dev/null; then
        local fish_completion_dir="$HOME/.config/fish/completions"
        mkdir -p "$fish_completion_dir"
        "$bin_path" --completions fish > "$fish_completion_dir/l0-compressor.fish"
        echo "complete -c t -w l0-compressor" > "$fish_completion_dir/t.fish"
        log_success "Fish completions generated in $fish_completion_dir/l0-compressor.fish and t.fish"
    fi

    # Bash
    local bash_completion_dir="$HOME/.local/share/bash-completion/completions"
    mkdir -p "$bash_completion_dir"
    "$bin_path" --completions bash > "$bash_completion_dir/l0-compressor"
    cat << 'EOF' > "$bash_completion_dir/t"
if [ -f "$HOME/.local/share/bash-completion/completions/l0-compressor" ]; then
    . "$HOME/.local/share/bash-completion/completions/l0-compressor"
fi
complete -F _l0-compressor t
EOF
    log_success "Bash completions generated in $bash_completion_dir/l0-compressor and t"
}

print_integration_info() {
    local abs_path=$1
    echo -e "\n${BOLD}${BLUE}======================================================================${NC}"
    echo -e "${GREEN}● 'l0-compressor' is installed and ready to use!${NC}"
    ...
    echo -e "   Once installed, 'l0-compressor' is available in your VS Code integrated terminal."
    echo -e "   Prefix your commands: ${YELLOW}l0-compressor cargo test${NC} or ${YELLOW}l0-compressor git diff${NC}."
    echo ""
    
    echo -e "2. ${BOLD}${GREEN}Claude Code (claudecode CLI)${NC}"
    echo -e "   Claude Code inherits your shell's environment PATH."
    echo -e "   Tell Claude Code to run commands prefixed with 'l0-compressor':"
    echo -e "   ${YELLOW}\"run: l0-compressor cargo test\"${NC} or ${YELLOW}\"run: l0-compressor npm test\"${NC}"
    ...
    echo -e "   To run tools under Claude Desktop using 'l0-compressor', configure your MCP tool"
    echo -e "   definition in ${YELLOW}~/Library/Application Support/Claude/claude_desktop_config.json${NC}."
    echo -e "   Reference the absolute path to 'l0-compressor':"
    echo -e "   ${YELLOW}\"$abs_path\"${NC} instead of \"l0-compressor\"."
    echo ""
    
    echo -e "4. ${BOLD}${GREEN}Claude CLI (Command Line)${NC}"
    echo -e "   The CLI tool operates directly in your shell."
    echo -e "   Prefix commands with 'l0-compressor' to keep Claude's context clean and save tokens."
    echo -e "${BOLD}${BLUE}======================================================================${NC}\n"
}
...
#!/bin/bash
# ==============================================================================
# l0-compressor - Git Pre-Commit Hook
# ==============================================================================
set -euo pipefail

# Print banner
echo -e "\033[1m\033[36m● Gating Quality & Security: Running l0-compressor Pre-Commit Checks...\033[0m"
...
# Check if we are inside the l0-compressor repository
is_in_repo() {
    if [ -f "Cargo.toml" ] && grep -q 'name = "l0-compressor"' Cargo.toml; then
        return 0
    fi
    return 1
}
...
if ! is_in_repo; then
    log_info "Not running inside the l0-compressor repository. Cloning from GitHub..."
    
    # Check if git is installed
    if ! command -v git &>/dev/null; then
        log_err "Git is required to clone and build l0-compressor. Please install git."
        exit 1
    fi
    
    TEMP_DIR=$(mktemp -d -t l0-compressor-install-XXXXXXXX)
    git clone --quiet https://github.com/fabriziosalmi/l0-compressor.git "$TEMP_DIR"
...
if [ ! -t 0 ]; then
    ...
    log_warn "This will clone l0-compressor, compile it with cargo, and install to $LOCAL_BIN_DIR."
...
echo -e "  4) ${BOLD}Uninstall${NC} 'l0-compressor' from the system"
